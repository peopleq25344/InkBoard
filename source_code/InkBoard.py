...
def pendraw(event):
    ...
    if (nx is None) and (ny is None):
        drawp.create_line(event.x, event.y, event.x, event.y, width=s_pt.get(), fill=cc)
        nx = event.x
        ny = event.y
        return
    drawp.create_line(nx, ny, event.x, event.y, width=s_pt.get(), fill=cc)
    nx = event.x
    ny = event.y
def pen():
    ...
    drawp.bind("<ButtonRelease>", penup)
...
clb = tk.Button(win, fg="#ffffff", bg="#404030", activebackground="#303030", activeforeground="#ffffff", relief="flat", bd=0, text="CLEAR", command=clean, font=("", 15))
clb.place(x=900, y=0)
